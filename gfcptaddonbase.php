	...
	class GFCPTAddonBase {
	  ...
	  protected $_tag_terms = array();
		...
		public function init() {
			...
			add_filter( 'gform_field_value_collaboration_select', array( &$this, 'exchange_populate_collaboration_select' ) );
		}
...
//
		...
		function load_posts_hierarchical( $post_type, $form_id, $field_id ) {
			...
			return $this->exchange_walk_posts( $posts );
		}
		...
		/*
		 * Helper function to recursively 'walk' the posts, edited to
		 */
		function exchange_walk_posts( $input_array, &$out_array=array(), $level=0 ){
			foreach ( $input_array as $item ) {
				$item->post_title = str_repeat('--', $level) . $item->post_title;
				$out_array[] = $item;
				$this->walk_posts( $input_array, $item->ID, $out_array, $level+1 );
			}
			return array_reverse( $out_array );
		}
		...
		function save_taxonomy_field( &$field, $entry, $taxonomy ) {
			...
			// Multiselect.
				...
				$term_ids = explode( ',', $entry[ $field['id'] ] );
				$term_id_int = intval( $term_id );
				if ( is_int( $term_id_int ) && 0 < $term_id_int ) {
					$term_id_arr[] = $term_id_int;
				}
			} else {
			...
			// This is the part where multiple Term ID's get stored via their slug
			if ( ! count( $term_id_arr ) ) {
				return;
			}
			...
			if ( !empty ( $term_slugs ) ) {
				wp_set_object_terms( $entry['post_id'], $term_slugs, $taxonomy, true );
			}
		}

		function exchange_populate_collaboration_select( $value, $field, $name ) {
		}
